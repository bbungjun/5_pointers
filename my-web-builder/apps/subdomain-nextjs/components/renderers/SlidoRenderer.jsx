...
function SlidoRenderer({ comp, pageId, mode = 'live', width, height }) {
  ...
  // 안전한 위치 생성 함수 (4분면 고르게 배치)
  const generateRandomPosition = (textSize, index = 0) => {
    ...
    // 각 분면의 크기 (여백 고려)
    const quadrantWidth = (containerWidth - totalWidth) / 2;
    const quadrantHeight = (containerHeight - totalHeight) / 2;
    ...
    switch (quadrant) {
      case 0: // 왼쪽 위
        x = margin + Math.random() * Math.max(quadrantWidth, 0);
        y = margin + Math.random() * Math.max(quadrantHeight, 0);
        break;
      case 1: // 오른쪽 위
        x = centerX + Math.random() * Math.max(quadrantWidth, 0);
        y = margin + Math.random() * Math.max(quadrantHeight, 0);
        break;
      case 2: // 왼쪽 아래
        x = margin + Math.random() * Math.max(quadrantWidth, 0);
        y = centerY + Math.random() * Math.max(quadrantHeight, 0);
        break;
      case 3: // 오른쪽 아래
        x = centerX + Math.random() * Math.max(quadrantWidth, 0);
        y = centerY + Math.random() * Math.max(quadrantHeight, 0);
        break;
      default:
        x = margin + Math.random() * Math.max(containerWidth - totalWidth, 0);
        y = margin + Math.random() * Math.max(containerHeight - totalHeight, 0);
    }

    // 경계 검사
    x = Math.max(margin, Math.min(x, containerWidth - totalWidth));
    y = Math.max(margin, Math.min(y, containerHeight - totalHeight));
  ...
  const createScatteredOpinions = (opinions, groups) => {
    ...
    return Array.from(uniqueOpinions.values()).map((opinion, index) => {
      ...
      while (attempts < 20 && !validPosition) {
        position = generateRandomPosition(textSize, index);
    ...
    >
      ...
      <div
        style={{
          fontSize:
            mode === 'live'
              ? `clamp(${Math.max(14, 20 * 0.7)}px, ${(20 / 375) * 100}vw, 20px)`
              : '20px',
          fontWeight: '700',
      ...
      {mode !== 'editor' && (
        ...
        >
          ...
          >
            <input
              type="text"
              placeholder={placeholder || '의견을 입력해보세요!'}
              value={newOpinion}
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              background:
                isSubmitting || !newOpinion.trim() ? '#d1d5db' : '#E91E63',
              color: '#ffffff',
              fontSize: '15px',
              fontWeight: '600',
              cursor:
                isSubmitting || !newOpinion.trim() ? 'not-allowed' : 'pointer',
              transition: 'all 0.3s ease',
              minWidth: '80px',
              boxShadow:
                isSubmitting || !newOpinion.trim()
                  ? 'none'
      ...
      >
        {mode === 'editor' ? (
          ...
          // 산재된 의견들 표시
          scatteredOpinions.map((opinion, idx) => {
            // 그룹 크기에 따른 스타일 계산
            const getOpinionStyle = (groupSize, index) => {
              const colors = [
                '#4CAF50', // 초록
                '#FF9800', // 주황
                '#9C27B0', // 보라
              ...
              const sizeIdx = Math.min(groupSize - 1, 4);

              return {
                background: color,
                fontSize: `${sizes[sizeIdx]}px`,
                padding: paddings[sizeIdx],
                transform: `rotate(${opinion.rotation}deg) scale(${opinion.scale * scales[sizeIdx]})`,
                boxShadow:
                  groupSize >= 3
            ...
            const style = getOpinionStyle(opinion.groupSize, idx);
