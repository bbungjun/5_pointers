...
app.use(async (req, res, next) => {
  ...
  // 직접 접근시 기본 페이지
  if (host.includes('43.201.8.232') || subdomain === 'localhost' || !host.includes('.')) {
    return res.send(`
...
# PM2로 서버들 실행
pm2 start yjs-server.js --name yjs-server
pm2 start subdomain-server.js --name subdomain-server
